...
class IssueDateTest {
    ...
    class NormalCase {
        @Test
        @DisplayName("有効な過去の日付で発行日を作成できる")
        void shouldCreateIssueDateWithPastDate() {
            // Given
            LocalDate pastDate = LocalDate.now().minusDays(1);

            // When
            IssueDate issueDate = IssueDate.create(pastDate);

            // Then
            assertNotNull(issueDate);
            assertEquals(pastDate, issueDate.value());
        }

        @Test
        @DisplayName("有効な今日の日付で発行日を作成できる")
        void shouldCreateIssueDateWithToday() {
            // Given
            LocalDate today = LocalDate.now();

            // When
            IssueDate issueDate = IssueDate.create(today);

            // Then
            assertNotNull(issueDate);
            assertEquals(today, issueDate.value());
        }
        ...
        void shouldReconstructIssueDateWithNull() {
            // When
            IssueDate issueDate = IssueDate.reconstruct(null);

            // Then
            assertNotNull(issueDate);
            assertNull(issueDate.value());
        }
    ...
    class AbnormalCase {
        @Test
        @DisplayName("未来の日付で発行日を作成しようとすると例外がスローされる")
        void shouldThrowExceptionWhenCreatingIssueDateWithFutureDate() {
            // Given
            ...
            DomainValidationException exception = assertThrows(
                    ...
                    () -> IssueDate.create(futureDate)
            );
            assertFalse(exception.getErrors().isEmpty());
            assertEquals("issueDate", exception.getErrors().get(0).field());
            assertEquals("validation.issueDate.future", exception.getErrors().get(0).messageKey());
        }

        @Test
        @DisplayName("nullで発行日を作成しようとすると例外がスローされる")
        void shouldThrowExceptionWhenCreatingIssueDateWithNull() {
            // When & Then
            DomainValidationException exception = assertThrows(
                    DomainValidationException.class,
                    () -> IssueDate.create((LocalDate) null)
            );
            assertFalse(exception.getErrors().isEmpty());
            assertEquals("issueDate", exception.getErrors().get(0).field());
        }
